...
barplot(ni,                                  #grafico a barre
        main = "Distribuzione Classi Sales",
        xlab = "Classi Sales",
...
barplot(efficacy_advertising_means,
        ylim = c(0,100),
        ylab = "Efficacia Advertising Percentuale",
        main = "Efficacia Advertising per Cittá",
...
ggplot(data = real_estate_texas)+    
  ...
  labs(
    ...
    title = "Valore vendite Anni/Città"
  )+
  labs(x="Years")
...
ggplot(real_estate_texas)+
  ...
  labs(x="Mesi",
       y="Totale Vendite",
       title="Andamento Vendite Immobiliari nel Tempo")+
  facet_wrap(~year)
